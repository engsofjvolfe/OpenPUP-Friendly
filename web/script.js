...
// =============================================
// SEÇÃO 1: Inicialização e Variáveis Globais
// =============================================
document.addEventListener("DOMContentLoaded", function () {
  // --- Variáveis Globais ---
  let languages = []; // Armazena a lista de idiomas carregada do JSON
  ...
  // --- Objeto com referências a todos os elementos do formulário ---
  const formElements = {
  ...
  function formatField(field) {
    return field && field.value ? field.value : "<em>Não definido</em>";
  }
  ...
  // --- Função genérica para criar e gerenciar tooltips ---
  function createTooltip(infoIcon) {
    ...
    function toggleTooltip(e) {
      ...
      if (!isVisible) {
        ...
        tooltip.offsetWidth; // Força reflow
        positionTooltip();
  ...
  // --- Construção do resumo para o modal de confirmação ---
  function buildModalSummary() {
    ...
    const resumoHTML = `
      <div class="info-box">
        ...
        <strong>Máx. de perguntas:</strong> ${
          formElements.maxQuestions.value
        }<br>
  ...
  // --- Exibição de sugestões de idioma ---
...
<!-- FIM DO PROMPT -->`;
  ...
  // =============================================
  // SEÇÃO 3: Carregamento de Dados e Eventos Iniciais
  // =============================================
  ...
  // --- Evento de input para sugestões de idioma ---
  ...
  if (introModal && closeIntroBtn && focusReturn) {
    const introModalController = setupModal(
      introModal,
      null, // Abre automaticamente
      "#close-intro",
      null, // Callback de abertura
      () => localStorage.setItem("introClosed", "true") // Callback de fechamento
    );

    // --- Abrir automaticamente se nunca foi fechado ---
    window.addEventListener("DOMContentLoaded", () => {
      const introClosed = localStorage.getItem("introClosed");
      if (!introClosed) {
        introModalController.openModal();
      }
    });
  ...
  if (confirmModal) {
    // Configura o modal e obtém as funções de controle
    const confirmModalController = setupModal(
      confirmModal,
      ".generate", // Botões que abrem o modal
      "#cancel-modal",
      () => {
        summaryBox.innerHTML = buildModalSummary();
      }, // Ao abrir
      null // Ao fechar
    );
    ...
    document
      .getElementById("confirm-generate")
      ?.addEventListener("click", () => {
        generateProtocol();
        confirmModalController.closeModal(); // Usa a função closeModal do controller
      });
  }

  // =============================================
  // SEÇÃO 6: Validação de Critérios
  // =============================================
  ...
  // --- Tooltip de ajuda para o campo de código ---
  codigoInput.title =
    "Digite apenas: M (Must), S (Should), A (Avoid) ou D (Data)";

  // --- Adicionar critério à tabela ---
  ...
  // =============================================
  // SEÇÃO 7: Reset e Botões de Ação
  // =============================================
  ...
  // =============================================
  // SEÇÃO 8: Copiar Protocolo (robusto, com fallback)
  // =============================================
  ...
  // garante que o elemento de feedback tem atributos ARIA (caso o HTML não tenha)
  if (copyFeedback) {
  ...
  function updateCopyState() {
    const content = formElements.preview.textContent.trim();
    if (copyBtn) copyBtn.disabled = content.length === 0;
  }

  // Observa mudanças no preview
  const observer = new MutationObserver(updateCopyState);
  if (formElements.preview) {
    observer.observe(formElements.preview, {
      childList: true,
      subtree: true,
      characterData: true,
    });
  ...
  // Caso generateProtocol dispare um evento, deixa fazer update também
  document.addEventListener("protocolGenerated", updateCopyState);

  // Função que tenta copiar de forma robusta (Clipboard API -> fallback textarea)
  async function copyToClipboard(text) {
    // Primeiro: Clipboard API (requer secure context: https ou localhost)
    ...
    // Fallback: textarea temporário + execCommand (mais compatível)
    try {
  ...
  // Mostra feedback visual e faz aria updates
  function showCopyFeedback(message, isSuccess = true) {
    if (!copyFeedback) return;
    copyFeedback.textContent = message;
    copyFeedback.classList.add("active");
    copyFeedback.setAttribute("aria-hidden", String(!isSuccess));
    setTimeout(() => {
      copyFeedback.classList.remove("active");
      copyFeedback.textContent = "";
      copyFeedback.removeAttribute("aria-hidden");
    }, 2000);
  }

  // Handler do botão copy
  copyBtn?.addEventListener("click", async () => {
    ...
    if (ok) {
      showCopyFeedback("Prompt copiado com sucesso!", true);
    } else {
      showCopyFeedback(
        "Erro ao copiar! Verifique console ou contexto (HTTPS/localhost).",
        false
      );
    }
  });
  ...
  // =============================================
  // SEÇÃO 9: Tooltips e Botões de Info
  // =============================================
  document.querySelectorAll(".info").forEach(createTooltip);
